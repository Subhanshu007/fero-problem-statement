...
<Header></Header>
  <div class="container">
    <table class="styled-table">
      ...
      <tbody>
        <tr v-for="(item, index) in citiesList" :key="`city-${index}`">
          <td>{{ item.name }}</td>
          <td>
            <button class="btn btn-primary" @click="provideWeatherDetails(item.name); showModal">Details</button>
          </td>
          <td>
            <svg
    ...
    <!-- modal starts -->
    <b-modal ref="weather-modal" hide-footer title="Feels Like">
      <div class="d-block text-center">
        {{weather}}
      </div>
      <b-button class="mt-2" variant="outline-warning" block @click="closeModal">Close Me</b-button>
    </b-modal>
    <!-- /modal ends -->
  </div>
</div>
...
export default {
  ...
  methods: {
   async provideWeatherDetails(cityName){
      const res = await this.$store.dispatch('getWeatherReport', cityName)
      this.weather = res.data.current.feelslike_c;
      this.$refs['weather-modal'].show()
      return this.weather;
    },
    closeModal(){
     return this.$refs['weather-modal'].hide();
    }
  },
  mounted() {
